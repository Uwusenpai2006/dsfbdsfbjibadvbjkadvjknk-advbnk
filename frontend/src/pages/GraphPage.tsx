  ...
  return (
    <div className="mt-2">
      ...
      <div className="relative h-12 bg-gray-900/50 rounded-lg overflow-hidden border border-gray-800/50">
          ...
          {data.map((d, i) => (
            <div
              ...
              style={{
                height: `${(d.y / maxY) * 100}%`,
                backgroundColor:
                  d.x >= beta ? "rgba(139,92,246,0.7)" : "rgba(75,85,99,0.4)",
              }}
            />
          ))}
        </div>
        ...
        >
          <div className="absolute -top-0.5 -translate-x-1/2 text-[8px] text-red-400 font-mono whitespace-nowrap">
            β={beta.toFixed(2)}
          </div>
        </div>
      </div>
            ...
            <div className="px-2.5 pb-2.5 grid grid-cols-2 gap-1.5 text-[10px]">
              <div className="p-1.5 bg-gray-900/50 rounded">
                ...
                <div className="font-mono text-gray-400">
                  {cluster.avg_out_degree}
                </div>
              </div>
              <div className="p-1.5 bg-gray-900/50 rounded">
                <div className="text-gray-600">In°</div>
                <div className="font-mono text-gray-400">
                  {cluster.avg_in_degree}
                </div>
              </div>
              <div className="p-1.5 bg-gray-900/50 rounded">
              ...
              <div className="p-1.5 bg-gray-900/50 rounded">
                ...
                <div className="font-mono text-gray-400">
                  {cluster.internal_weight.toFixed(1)}
                </div>
              </div>
                ...
                <div className="col-span-2 p-1.5 bg-gray-900/50 rounded">
                  ...
                  <div className="flex flex-wrap gap-1">
                    {cluster.hub_neurons.map((h) => (
                      <span
                        key={h.neuron}
                        className="px-1 py-0.5 rounded bg-gray-800 font-mono text-gray-500"
                      >
                        #{h.neuron}
                      </span>
                    ))}
...
export function GraphPage() {
  ...
  const loadClusters = useCallback(
    async (head: number, b: number, tries = 0) => {
      setIsLoading(true);
      setError(null);
      ...
      try {
        ...
        const res = await api.get(`/graph/clusters/french`, {
          params: { head, beta: b, max_nodes: 400 },
          timeout: 60000,
          cancelToken: cancel.token,
        });
        // Defensive: handle missing/partial data
        if (!res.data || !res.data.nodes || !res.data.edges) {
          throw new Error("Malformed cluster data");
        }
        ...
        if (err?.message?.includes("cancel")) {
          setError("Cluster loading cancelled.");
        } else if (tries < 2) {
          setError("Cluster loading failed, retrying...");
          setRetryCount(tries + 1);
          setTimeout(() => loadClusters(head, b, tries + 1), 1000 * (tries + 1));
        } else {
          setError(
  ...
  useEffect(() => {
    ...
    betaTimeout.current = setTimeout(
      () => loadClusters(selectedHead, beta),
      500,
    );
  ...
  const runActivation = useCallback(async () => {
    ...
    setBackendOffline(false);
    try {
      const res = await api.post("/graph/activate", {
        text: inputText,
        model_name: "french",
        head: selectedHead,
        layer: selectedLayer,
      ...
      setError(
        err?.response?.data?.detail || err?.message || "Activation failed"
      );
      setBackendOffline(true);
    } finally {
  ...
  useEffect(() => {
    ...
    if (
      ...
      activationResult &&
      playbackIdx >= 0 &&
      playbackIdx < activationResult.num_tokens
    ) {
  ...
  const currentTokenActivation = useMemo(() => {
    if (
      !activationResult ||
      playbackIdx < 0 ||
      playbackIdx >= activationResult.per_token.length
    )
      return null;
      ...
      ? clusterData.edges.map((e) => ({
          source:
            typeof e.source === "object"
              ? (e.source as ClusterNode).id
              : e.source,
          target:
            typeof e.target === "object"
              ? (e.target as ClusterNode).id
              : e.target,
          weight: e.weight,
  ...
  const linkColor = useCallback(
    (link: any) => {
      ...
      if (link.same_cluster) {
        const src = clusterData?.nodes.find(
          (n) =>
            n.id ===
        ...
        if (src) {
          ...
          const dimmed =
            highlightCluster !== null && src.cluster !== highlightCluster;
          return dimmed ? "rgba(40,40,55,0.04)" : `${c}44`;
        }
      }
      return highlightCluster !== null
        ? "rgba(40,40,55,0.03)"
        : "rgba(120,120,150,0.12)";
    },
  ...
  return (
    <div className="h-screen flex flex-col lg:flex-row overflow-hidden">
      ...
      {backendOffline && (
        <div className="w-full bg-red-900 text-red-200 text-xs py-2 px-4 font-mono flex items-center gap-2 z-50">
          <AlertCircle size={16} />
          Backend offline or unresponsive. Check server, retry, or use playback mode.
          <button
            className="ml-auto bg-red-700 px-2 py-1 rounded text-xs hover:bg-red-600"
            onClick={() => {
              setBackendOffline(false);
              setRetryCount(0);
              loadClusters(selectedHead, beta);
      ...
      <div className="flex-1 flex flex-col min-w-0 overflow-hidden">
        ...
        <div className="px-5 pt-4 pb-1 flex-shrink-0">
          ...
          <p className="text-gray-600 text-xs mt-0.5">
            G* = D<sub>x</sub>E · Louvain community detection on trained french
            model
          </p>
        </div>
        ...
        <div className="px-5 py-2 flex flex-wrap items-center gap-2.5 flex-shrink-0">
          ...
          <div className="flex items-center gap-1.5 flex-1 max-w-[220px]">
            ...
            />
            <span className="text-[10px] font-mono text-bdh-accent w-9 text-right">
              {beta.toFixed(3)}
            </span>
          </div>
          <div className="w-px h-5 bg-gray-800" />
          ...
          <button
            ...
            className="p-1.5 rounded-md bg-gray-800/50 text-gray-500 hover:bg-gray-700 transition-all"
          >
            <RotateCcw size={14} />
          </button>
        </div>
          ...
          {isLoading ? (
            <div className="absolute inset-0 flex items-center justify-center z-10">
              <div className="text-center">
                <Loader2 className="w-8 h-8 text-bdh-accent animate-spin mx-auto mb-2" />
                <p className="text-gray-600 text-xs">Computing clusters...</p>
              </div>
            </div>
            ...
            <ForceGraph3D
              ...
              controlType="orbit"
              nodeLabel={(node: any) =>
                `#${node.id} · Cluster ${node.cluster} · Deg ${node.degree}${node.is_hub ? " (HUB)" : ""}`
              }
              onNodeClick={(node: any) =>
                setHighlightCluster((prev) =>
                  prev === node.cluster ? null : node.cluster,
                )
              }
          ...
          {clusterData && !isLoading && (
            <div className="absolute top-2 left-2 flex gap-1.5 pointer-events-none">
              ...
              <div className="px-2 py-1 rounded-md bg-black/70 backdrop-blur text-[10px] font-mono text-bdh-accent/80 border border-gray-800/40">
                {clusterData.num_clusters} clusters · Q=
                {clusterData.modularity.toFixed(3)}
              </div>
            </div>
          )}
                ...
                <div className="flex items-center gap-2">
                  ...
                  <div className="flex gap-0.5 flex-wrap">
                    {activationResult?.input_chars.map((ch, i) => (
                      <span
                        key={i}
                        className={`px-1 py-0.5 rounded text-xs font-mono transition-all duration-200 ${
                          i === playbackIdx
        ...
        <div className="px-5 pb-4 flex-shrink-0">
          <div className="glass-card p-3">
            <div className="flex items-center gap-1.5 mb-2">
              ...
              <span className="text-[10px] text-gray-600">
                — activations play character by character
              </span>
              {/* Playback controls */}
              {activationResult && (
                <div className="ml-auto flex items-center gap-1">
                  <button
                    ...
                    className="p-1 rounded bg-gray-800 hover:bg-gray-700 text-gray-400 hover:text-gray-200 transition-all"
                    title="Replay"
                  >
                    <RotateCcw size={12} />
                  </button>
                  ...
                  <button
                    onClick={() => {
                      ...
                      setPlaybackIdx((prev) =>
                        Math.min(
                          prev + 1,
                          (activationResult?.num_tokens || 1) - 1,
                        ),
                      );
                    }}
                    className="p-1 rounded bg-gray-800 hover:bg-gray-700 text-gray-400 hover:text-gray-200 transition-all"
            ...
            <div className="mt-1.5 flex flex-wrap gap-1">
              {[
                ...
                <button
                  ...
                  className="px-1.5 py-0.5 text-[9px] bg-gray-800/60 hover:bg-gray-700 rounded text-gray-600 hover:text-gray-400 transition-colors truncate max-w-[180px]"
                >
                  {ex}
                </button>
              ))}
      ...
      >
        <div className="p-3 border-b border-gray-800/50 flex-shrink-0">
          <div className="flex items-center justify-between">
            <h2 className="text-xs font-bold text-gray-400 uppercase tracking-wider flex items-center gap-1.5">
              <Activity size={12} className="text-bdh-accent" />
              Clusters
            </h2>
            {clusterData && (
              <span className="text-[9px] font-mono text-gray-700">
                {clusterData.n_neurons.toLocaleString()} neurons
              </span>
            )}
          </div>
          ...
          {currentTokenActivation && (
            <div className="mt-2 px-2 py-1.5 rounded-lg bg-bdh-accent/10 border border-bdh-accent/20">
              <div className="text-[9px] text-bdh-accent/60 uppercase tracking-wider">
                Token {currentTokenActivation.token_idx + 1}/
                {activationResult?.num_tokens}
              </div>
              <div className="text-sm font-mono text-white mt-0.5">
                '{currentTokenActivation.char}'{" "}
                <span className="text-gray-500 text-[10px]">
                  byte {currentTokenActivation.byte}
                </span>
              </div>
            </div>
          )}
        ...
        {clusterData && (
          <div className="px-3 flex-shrink-0">
            <MiniHistogram
              data={clusterData.histogram}
              beta={beta}
              label="G* element distribution"
            />
          </div>
        )}
          ...
          {clusterData?.clusters
            ...
            .map((cluster) => (
              <ClusterPill
                ...
                cluster={cluster}
                activation={
                  currentClusterMap[cluster.cluster_id] ??
                  (activationResult
                    ? (activationResult.cumulative_cluster_activations.find(
        ...
        <div className="p-3 border-t border-gray-800/50 bg-gray-950/50 flex-shrink-0">
          <p className="text-[10px] text-gray-600 leading-relaxed">
            ...
            via Louvain. Head {selectedHead + 1}/4 · β={beta.toFixed(3)}. Type
            text and watch clusters blink as each character is processed.
          </p>
        </div>
      </motion.aside>
    </div>
