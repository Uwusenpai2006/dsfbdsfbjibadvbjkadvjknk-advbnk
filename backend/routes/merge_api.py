...
class HeritageProbeRequest(BaseModel):
    ...
    model_name: str = Field(default="merged_finetuned", description="Merged model to probe")
...
def heritage_probe(request: HeritageProbeRequest, req: Request):
    ...
    try:
        ...
        # Fallback chain
        for fallback in ["merged_finetuned", "merged_polyglot"]:
    ...
    if not heritage:
        raise HTTPException(400, f"Model '{model_name}' has no heritage info — is it a merged model?")
    ...
    # Summary
    all_fr = sum(d["french"]["activation_ratio"] for d in layers_result.values())
    all_pt = sum(d["portuguese"]["activation_ratio"] for d in layers_result.values())
    total = all_fr + all_pt
